-- Global key bindings
-- -----------------------------------------------------------------------------
-- TODO: Maybe redo so they can be accessed one-handedly?
local globalkeys = awful.util.table.join(
    ...
    awful.key({globals.modkey, "Control"}, "Left", function()
        -- awful.client.focus.history.previous()
        awful.client.focus.byidx(-1)
        if client.focus then client.focus:raise() end
    end),
    awful.key({globals.modkey, "Control"}, "Right", function()
        -- awful.client.focus.history.previous()
        awful.client.focus.byidx( 1)
        if client.focus then client.focus:raise() end
    end),
...
for i = 1, #tags do
    globalkeys = awful.util.table.join(globalkeys,
        ...
        -- View tag (exclusive).
        awful.key({globals.modkey}, "#" .. i + #tags, function()
            local tag = awful.tag.gettags(mouse.screen)[i]
            if tag then awful.tag.viewonly(tag) end
        end),

        -- View tag (cumulative).
        awful.key({globals.modkey, "Control"}, "#" .. i + #tags, function()
            local tag = awful.tag.gettags(mouse.screen)[i]
            if tag then awful.tag.viewtoggle(tag) end
        end),

        -- Move client to tag.
        awful.key({globals.modkey, "Shift"}, "#" .. i + #tags, function()
        ...
        -- Add client to tag.
        awful.key({globals.modkey, "Control", "Shift"}, "#" .. i + #tags, function()
            if client.focus then
                local tag = awful.tag.gettags(client.focus.screen)[i]
...
local clientkeys = awful.util.table.join(
  ...
  --awful.key({globals.modkey, "Control"}, "space",  awful.client.floating.toggle                    ),
    ...
    awful.key({globals.modkey           }, "s",      function(c) c.minimized = true               end), -- The client currently has the input focus, so it cannot be minimized, since minimized clients can't have the focus.
    awful.key({globals.modkey           }, "m",      function(c)
